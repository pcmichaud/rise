...
# scenario
scenarios = ['inc50','reference','inc150']
scnnames = ['$\overline{m}(-50)$','$\overline{m}(ref)$','$\overline{m}(+50)$']
inc = [0.5,1,1.5]
enames = ['e(50)','e(100)']
elatex = ['$\epsilon_y(-50)$','$\epsilon_y(+50)$']
# setting directory
...
income =pd.DataFrame(index=agenames,columns=scenarios)
...
for scn in scenarios :
	...
	df['age5'] = pd.cut(df['age'], 8,labels=agenames)
	result[scn] = df.groupby(['age5'])['medexp'].mean()
	income[scn] = df.groupby(['age5'])['income'].mean()
	if (ss!=0):
		lastscn = scenarios[ss-1]
		c = inc[ss-1]*100
		var = 'e('+str(int(c))+')'
		dincome = (income[scn] - income[lastscn])/(0.5*(income[scn]+income[lastscn]))
		result[var] = (result[scn] - result[lastscn])/(0.5*(result[scn]+result[lastscn]))
		result[var] = result[var]/dincome
	ss +=1


tex = open('../tables/table-income-elasticity.tex', "w")
col = 'l'
